...
//		model.addAttribute("size2", s2);
	...
	public String rechercherCommandesMedicament(Model model,
			                                    @RequestParam(name="motCle", defaultValue="") String mc,
			                                    @RequestParam(name="page", defaultValue="0") int p,
			                                    @RequestParam(name="size", defaultValue="4") int s) {
		Page<LigneCommande> pagesLignesCommandes=pharmacieMetierImpl.consulterLignesCommandesMedicament(mc, p, s);
		model.addAttribute("lignesCommandes", pagesLignesCommandes.getContent());
		int[] pages=new int[pagesLignesCommandes.getTotalPages()];
		model.addAttribute("pages", pages);
		model.addAttribute("pageCourante", p);
		model.addAttribute("motCle", mc);
		model.addAttribute("size", s);
	...
	public String supprimerMedicament(Long code) {
		pharmacieMetierImpl.deleteMedicament(code);
		return "pharmacie";
	}
	...
	public String passerVente(Model model, Long codeMedicament) {
		Medicament medicamentChoisi=pharmacieMetierImpl.consulterMedicamentAvecId(codeMedicament);
		model.addAttribute("medicamentChoisi",medicamentChoisi);
		List<Medicament> listMedicaments=pharmacieMetierImpl.listMedicaments();
		model.addAttribute("listMedicaments",listMedicaments);
		return "formVente";
	}
	//méthode process vente
	@RequestMapping(value="/processPasserVente",method=RequestMethod.POST)
	public String confirmationVente(Model model, Vente vente, String libelleMedicament, int quantiteMedicament) {
		
		//liste stocks medicament si insuffisant
       //option ajouter autre medicament+verification disponiblité stocks..
		
		return "confirmationVente";
	}
